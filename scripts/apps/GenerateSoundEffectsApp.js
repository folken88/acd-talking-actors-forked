...
export class GenerateSoundEffectsApp extends Application {
    ...
    activateListeners(html) {
        ...
        $('[name=ta_sf_repeat]').change($.proxy(async function (e) {
            that.loop = $(e.currentTarget).is(':checked');
        }, this));
    ...
    async generateEffectInternal() {
        let response = await game.talkingactors.connector.generateSoundEffect(this.text, { durationSeconds: this.durationSeconds, promptInfluence: this.promptInfluence, loop: this.loop }, this.filename);
