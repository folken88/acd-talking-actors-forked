...
export class ElevenlabsConnector {
    ...
    hasApiKey() {
        const apiKey = game.settings.get(MODULE.ID, MODULE.APIKEY);
        const masterApiKey = game.settings.get(MODULE.ID, MODULE.MASTERAPIKEY);
        
        return (apiKey?.length > 1) || (masterApiKey?.length > 1);
    }
    ...
    processChatMessage(chatlog, messageText, chatData, postToChat = true) {
        ...
        if (messageData = messageText.match("^\\[([a-zA-z0-9]+)\\] ((.|[\r\n])*)$")) {
            ...
            // Use default values for voice name override since we're not getting settings from an actor
        ...
        // otherwise check if an actor has been spefied for the talk command, e.g. "/talk {Actor} ..."
            ...
            if (speakerActor) {
                ...
                ({ voice_id, settings, modelId, languageCode, dialogueMode, audioTags } = this.getVoiceIdAndSettingsFromActor(speakerActor));
            }
        }
        // otherwise
        else {
        ...
        if (voice_id) {
            ...
            this.textToSpeech(voice_id, messageText, settings, chatlog, chatMessagePromise, modelId, languageCode, dialogueMode, audioTags);
        } else {
    ...
    getVoiceIdAndSettingsFromActor(speakerActor) {
        ...
        if (speakerActor) {
            ...
            modelId = moduleFlags ? moduleFlags[FLAGS.MODEL_ID] : "eleven_turbo_v2";
            languageCode = moduleFlags ? moduleFlags[FLAGS.LANGUAGE_CODE] : "en";
            dialogueMode = moduleFlags ? moduleFlags[FLAGS.DIALOGUE_MODE] : false;
            audioTags = moduleFlags ? moduleFlags[FLAGS.AUDIO_TAGS] : null;
        }
        return { voice_id, settings, modelId, languageCode, dialogueMode, audioTags };
    }
    ...
    async getVoices() {
        try {
            ...
            console.error("Talking Actors: Error loading voices:", error);
            this.allVoices = [];
        }
    }
    ...
    async textToSpeech(voiceId, text, settings, chatlog, chatMessagePromise, modelId = "eleven_turbo_v2", languageCode = "en", dialogueMode = false, audioTags = null) {
        let container = await new TextToSpeechRequest(voiceId, text, settings, modelId, languageCode, dialogueMode, audioTags).fetch();

        if (!container.ok) {
            console.error(`Talking Actors: TTS failed for voice ${voiceId} with status ${container.status}`);
            return;
        }
