...
export class ElevenlabsRequest {
    ...
    async postData(command, acceptType, body) {
        ...
        // Add error handling for 403 Forbidden (restricted voices)
        if (!response.ok && response.status === 403) {
            const errorText = await response.text();
            console.error(`Talking Actors: 403 Forbidden - This voice may require a higher subscription tier or be restricted`);
        }
